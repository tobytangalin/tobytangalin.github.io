...
## Search
- Client-side only (PaperMod + Fuse.js). `/search/` (`content/search.md`, `layout: search`) fetches `/index.json`.
- `/index.json` is generated by the project override `layouts/index.json` from `site.RegularPages`, with **title + summary only** (post bodies are intentionally not indexed). Pages excluded from collections (`build.list: never`) or with `searchHidden: true` don't appear — that's how the wedding stays out of search.
- Relevance is tuned in `[params.fuseOpts]` (use **lowercase** keys; Hugo lowercases config keys and the JS reads them lowercase). Current: `threshold = 0.15`, `ignorelocation = true`, `keys = ["title","summary"]`, `minmatchcharlength = 3`. Don't raise the threshold or re-add `content` — the old `0.4` + full-body index made `selen` match every post.
...
- Generated feeds, `sitemap.xml`, `index.json`, and listing pages — derive from `content/`.
